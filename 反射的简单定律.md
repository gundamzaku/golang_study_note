...
////
从最基本的讲起，反射仅是一种检查类型和接口变量内部数值双组数据形式存诸在接口变量中的机制。一开始，在反射包中我们知道有两种类型：Type和Value。这两个类型可以访问接口变量的内容，此外还有两个方法，称之为reflect.TypeOf和reflect.ValueOf，用于从一个接口值中取到reflect.Type和reflect.Value的结构。（同样，从reflect.Value中得到reflect.Type也是相当容易的，但是我们现在要区别并保有Value和Type两个不同的概念）  
