...
其实我们可以做得更好，传入到fmt.Println,fmt.Printf等变量都可以是空的接口数值，之后它会在fmt包的内部进行解包操作，正如我们在上一个例子中做的一样。它可以正确地打印reflect.Value的内容来传递接口方法的结果给格式化打印程序。  
...
再次说明，这里不需要做类型断言来判断v.Interface()的结果为float64；空接口值内有具体数值的类型信息，在打印的时候会恢复它。  
...
简而言之，这接口方法是函数ValueOf的反逆，除非ValueOf的结果永远是静态类型interface{}  
...
这个问题在于7.1这个数值并没有可设定的地址；也就是说目前的v是不可设置的。Settability`可设置性`是反射值的一个特性，但并非所有的反射数值都具有这个特性。  
...
是被允许并且是成功执行的，它并非更新变量x，即使v看上去像是从变量x那里被创建的，它将更新存储在反射值内的x的拷贝，而x自身不受到任何影响。这将非常的混乱和无用，因此他是不合法的，settability则正是为了避免这种问题而产生的一种特性。  
...
虽然这看上去非常奇怪，实际并非这样。这只是包了一层怪异的外衣的迷惑，事实上还是我们所熟悉的一种情形。想一下在函数中传入x：  
...
反射对象p是不可设置的，但是我们想设置的并不是p，而是p的指针。我们调用值中的Elem方法来得到p所指向的地址，它直接通过指针，并在称为v的反射值中保留结果：  
...
反射不容易被理解，但是它所做的事情切实是程序语言所能做的事情，虽然通过反射的Types和Values能掩饰所发生的过程，只要记住反射的Values为了修改它们所表示的内容时需要内容的地址。  
