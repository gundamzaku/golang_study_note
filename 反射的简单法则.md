...
当我们调用reflect.TypeOf(x)方法的时候，x首先存储在一个空接口中，并经过传递；reflect.TypeOf解包空接口并恢复其所类型信息。  
...
reflect.ValueOf方法，当然就是把这个值给恢复出来（这里我们省略了一些样本，将专注于执行的代码）    
...
value: <float64 Value>
```
（我们明确的调用这个String方法，因为通过默认的fmt包深入到reflect.Value中去显示其中的具体数值，字符串方法并非如此。）  
...
在反射库里有一组特性值得重点对待。首先，为保持API的简洁性，在“getter”和“setter”两个操作方法中都是以最大的类型定义去持有（或是操作）数值的：如int64用在所有的有符号整型中。所以，Value中的Int方法一律返回int64类型，并且在SetInt进行赋值时也采用int64类型进行操作；这在转换成实际类型时是非常有必要的。  
...
这个变量v的Kind仍旧是reflect.Int，尽管x的静态类型是MyInt，而不是int。换种说法，这Kind不能从MyInt中辨别出int，哪怕Type可以区分。
...
通过reflect.Value我们可以使用接口方法来恢复接口的值;事实上此方法将类型和值的信息打成一个以接口形式展呈的包数据，并作为结果返回：
