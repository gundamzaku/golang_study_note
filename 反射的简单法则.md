...
一个重要的细节是这个接口中的对偶总是以（值，具体实现类型）这样的形式呈现，而不能以（值，接口类型）的形式呈现。接口不能持有接口值。  
...
在最基本的水平，反射仅是一种检查类型和接口变量内部的值对偶存储的机制。一开始，在反射包中我们知道有两种类型：Type和Value。这两个类型可以访问接口变量的内容，此外还有两个方法，称之为reflect.TypeOf和reflect.ValueOf，用于从一个接口值中取到reflect.Type和reflect.Value的块段。（同样，从reflect.Value中得到reflect.Type也是相当容易的，但是我们现在要区别并保有Value和Type两个不同的概念）  
...
type: float64
```
你可以惊异于此处的interface，从这段程序中看经过reflect.TypeOf，显示出的是浮点变量x，而并非是接口值。但是在这里;参考go的文档报告，reflect.TypeOf的签名中包含和空接口：  
...
```
当我们调用reflect.TypeOf(x)方法的时候，x首先存储在一个空接口中，并经过论据通过；reflect.TypeOf解开空接口并恢复类型信息。  
...
value: <float64 Value>
```
（我们明确的调用这个String方法，因为在默认的fmt包深掘reflect.Value去显示具体的值，而字符串方法不行。）
